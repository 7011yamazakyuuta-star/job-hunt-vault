...
import { hmacSha256Hex, randomTokenHex, sha256Hex, timingSafeEqualText } from "./crypto";
...
describe("security helpers", () => {
  ...
  it("uses keyed hashes for session tokens", async () => {
    const digest = await hmacSha256Hex("session-token", "test-secret");
    expect(digest).toMatch(/^[a-f0-9]{64}$/);
    expect(digest).not.toBe(await sha256Hex("session-token"));
  });
  ...
  it("verifies room passphrases against salted hashes", async () => {
    ...
    expect(stored.hash).toMatch(/^pbkdf2-sha256:\d+:[a-f0-9]{64}$/);
