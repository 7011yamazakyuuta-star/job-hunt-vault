...
Use `GET /api/company-catalog/search` for local catalog lookup, `GET /api/logo/resolve` for domain-based logo URLs, and `GET /api/logo/search` for provider-backed logo candidates. Catalog search supports industry filtering and `sort=industry` or `sort=ticker`. Non-empty searches merge D1 catalog rows with a 117-record built-in Japanese company and organization catalog with common-name aliases, so core aliases such as Tokyo Metro remain available even when imported data is partial. See `docs/DATA_CATALOG.md`.
