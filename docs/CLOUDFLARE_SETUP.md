...
2. 個人ルーム、または共有ルームを作る。
3. 企業台帳で企業を1件追加する。
4. 進捗タブで、その企業に選考状態を1件保存する。
5. 日程タブで、その企業に予定またはTODOを1件保存する。
6. 適性検査タブで、その企業にレポートを1件保存する。
7. 金庫タブで任意のテスト用パスフレーズを入力し、テスト用メモを保存する。
8. 同じパスフレーズで金庫アイテムを復号できるか確認する。

注意:

- 金庫パスフレーズ、ログインID、パスワード、実際のMyPage情報はこのチャットに貼らないでください。
- 金庫パスフレーズはブラウザ内の暗号化だけに使われ、Workerには送信されません。
- 最初は架空の企業名やテスト用メモで確認してください。
