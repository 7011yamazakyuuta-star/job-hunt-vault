...
JPX の上場会社一覧、証券コード、市場区分、業種などは、この `company_catalog` に取り込みます。選考データとは別なので、後から辞書を更新してもユーザーの進捗やメモには触りません。
...
2. import scriptで `company_catalog` 用のSQLまたはD1 batchへ変換する
3. D1に投入する
